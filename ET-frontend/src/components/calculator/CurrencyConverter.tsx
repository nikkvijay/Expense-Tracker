...
interface CurrencyConverterProps {
  onConversionResult?: (amount: number, fromCurrency: string, toCurrency: string) => void;
}

export const CurrencyConverter: React.FC<CurrencyConverterProps> = ({ onConversionResult }) => {
  ...
  const [rateInfo, setRateInfo] = useState<{ rate: number; inverse: number } | null>(null);
  ...
  const fetchExchangeRates = async () => {
    try {
      ...
      toast.error('Failed to fetch live exchange rates. Using approximate rates.');
  ...
  const getFallbackRates = (baseCurrency: string): ExchangeRates => {
    ...
    const baseRates: { [key: string]: ExchangeRates } = {
      USD: { EUR: 0.85, GBP: 0.73, INR: 83.12, JPY: 110.0, CAD: 1.25, AUD: 1.35, CHF: 0.88, USD: 1.0 },
      EUR: { USD: 1.18, GBP: 0.86, INR: 97.8, JPY: 129.5, CAD: 1.47, AUD: 1.59, CHF: 1.04, EUR: 1.0 },
      INR: { USD: 0.012, EUR: 0.010, GBP: 0.0088, JPY: 1.32, CAD: 0.015, AUD: 0.016, CHF: 0.011, INR: 1.0 },
      GBP: { USD: 1.37, EUR: 1.16, INR: 113.8, JPY: 150.7, CAD: 1.71, AUD: 1.85, CHF: 1.21, GBP: 1.0 }
    };
      ...
      <CardContent className="space-y-4">
        ...
        {rateInfo && (
          <div className="bg-secondary/30 p-3 rounded-lg space-y-2">
            ...
            <div className="text-xs text-muted-foreground space-y-1">
              <div>1 {fromCurrency} = {rateInfo.rate.toFixed(4)} {toCurrency}</div>
              <div>1 {toCurrency} = {rateInfo.inverse.toFixed(4)} {fromCurrency}</div>
            </div>
          </div>
        )}
        ...
        <div>
          ...
          <div className="grid grid-cols-4 gap-2 mt-2">
            {['10', '100', '1000', '5000'].map((quickAmount) => (
              <Button
                key={quickAmount}
                variant="outline"
                size="sm"

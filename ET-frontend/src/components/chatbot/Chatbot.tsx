...
import { MessageCircle, Send, Bot, User, X, Trash2, HelpCircle, Mic, MicOff, Volume2 } from 'lucide-react';
import { sendChatMessage, getChatHistory, clearChatHistory, getChatbotCapabilities, speechToText, getSpeechServiceStatus, type ChatMessage, type ChatResponse } from '@/api';
...
export const Chatbot: React.FC<ChatbotProps> = ({ 
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.actionResult?.success) {
        ...
        } else if (response.action?.type === 'DELETE_EXPENSE' && onExpenseAdded) {
          onExpenseAdded();
  ...
  const getWelcomeMessage = () => {
    return `👋 Hi! I'm your personal finance assistant. I can help you manage your expenses and income using natural language. Try saying things like:\n\n• "I spent ${formatCurrency(25)} on lunch"\n• "Add ${formatCurrency(3000)} salary income"\n• "Show my expenses"\n• "Analyze my spending"\n\n${isVoiceAvailable ? '🎤 You can also use voice messages by clicking the microphone button!' : ''}\n\nWhat would you like to do?`;
  };
  ...
  const handleVoiceToText = async (audioBlob: Blob) => {
    ...
    try {
      ...
      if (response.transcript) {
        ...
        toast.success('✅ Speech converted to text! Edit if needed, then send.');
        
        // Show confidence if low
        if (response.confidence < 0.8) {
          toast.warning(`Voice recognition confidence: ${Math.round(response.confidence * 100)}%`);
        }
      } else {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-full max-w-2xl h-[80vh] flex flex-col p-0 bg-card border-0 shadow-large">
        ...
        {showHelp && capabilities && (
          <div className="p-4 bg-surface border-b border-border/50">
            <h4 className="font-semibold mb-2 text-foreground">What I can help you with:</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
              {capabilities.intents?.slice(0, 4).map((intent: any, index: number) => (
                <div key={index} className="text-muted-foreground">
                  <span className="font-medium text-foreground">{intent.name}:</span> "{intent.examples[0]}"
                </div>
              ))}
            </div>
          </div>
                  ...
                  <div className="flex gap-3">
                    <div className="flex-shrink-0">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                        message.success ? 'bg-primary/10' : 'bg-destructive/10'
                      }`}>
                        <Bot className={`h-4 w-4 ${
                          message.success ? 'text-primary' : 'text-destructive'
                        }`} />
                      </div>
                    </div>
                    <div className="flex-1">
          ...
          {isRecording && (
            <div className="mb-4 p-3 bg-primary/10 border border-primary/20 rounded-lg">
              ...
              <p className="text-xs text-muted-foreground mt-2">
                Speak clearly - your speech will be converted to text for editing...
              </p>
            </div>
          )}
          ...
          <form onSubmit={handleSendMessage} className="flex gap-2">
            ...
            <div className="flex-1 relative">
              <Input
                ...
                placeholder={`Type your message${isVoiceAvailable ? ' or click mic to speak' : ''}... (e.g., 'I spent ${formatCurrency(25)} on lunch')`}
